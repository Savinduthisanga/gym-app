        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              className={`w-full bg-gray-800 border rounded-lg px-3 py-2.5 text-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm ${errors.date ? 'border-red-500' : 'border-gray-700'}`}
            />
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{meals.length} meals logged total</p>
        </div>
        <button
      ...
      <div className="flex items-center gap-3">
        <label className="text-gray-400 text-sm">View date:</label>
        <input
          type="date"
          value={selectedDate}
          onChange={e => setSelectedDate(e.target.value)}
          className="bg-gray-900 border border-gray-800 rounded-lg px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
        />
      </div>

      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-white font-semibold">Daily Summary — {selectedDate}</h2>
          <span className={`text-sm font-bold ${caloriePercent >= 100 ? 'text-red-400' : 'text-orange-400'}`}>
      ...
      <div className="space-y-3">
        ...
        {todayMeals.length === 0 ? (
          <div className="bg-gray-900 border border-gray-800 rounded-2xl text-center py-12 text-gray-500">
            ...
            <button onClick={() => setShowModal(true)} className="text-orange-400 text-sm mt-2 hover:underline">Log a meal</button>
          </div>
        ) : (
          <div className="space-y-3">
            {todayMeals.map(m => (
              <div key={m.id} className="bg-gray-900 border border-gray-800 rounded-xl p-4 flex items-center justify-between gap-4 hover:border-gray-700 transition group">
                ...
                <div className="flex items-center gap-6 flex-shrink-0">
                  <div className="hidden sm:flex gap-4 text-xs">
                    <span className="text-blue-400"><span className="font-bold">{m.protein}g</span> protein</span>
                    <span className="text-yellow-400"><span className="font-bold">{m.carbs}g</span> carbs</span>
                    <span className="text-red-400"><span className="font-bold">{m.fat}g</span> fat</span>
                  </div>
                  <div className="text-center">
                    <p className="text-orange-400 font-bold">{m.calories}</p>
                    <p className="text-gray-600 text-xs">kcal</p>
                  </div>
                  <button onClick={() => removeMeal(m.id)} className="text-gray-700 hover:text-red-400 transition text-lg opacity-0 group-hover:opacity-100">🗑️</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {allDates.length > 1 && (
        <div className="space-y-3">
          <h2 className="text-white font-semibold text-lg">History by Day</h2>
                ...
                {allDates.map(date => {
                  ...
                  return (
                    <tr
                      ...
                      className={`hover:bg-gray-800/50 transition cursor-pointer ${date === selectedDate ? 'bg-orange-500/10 border-l-2 border-orange-500' : ''}`}
                    >
                      <td className="px-4 py-3 text-white font-medium">{date}</td>
                      <td className="px-4 py-3 text-gray-300">{dayMeals.length}</td>
                      <td className="px-4 py-3 text-orange-400 font-medium">{cal} kcal</td>
                      <td className="px-4 py-3 text-blue-400 hidden sm:table-cell">{prot}g</td>
                      <td className="px-4 py-3 text-yellow-400 hidden sm:table-cell">{carb}g</td>
                      <td className="px-4 py-3 text-red-400 hidden sm:table-cell">{fat}g</td>
                    </tr>
                  );
                })}
              </tbody>
