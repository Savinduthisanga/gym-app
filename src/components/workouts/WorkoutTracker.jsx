    ...
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 px-4">
      <div className="bg-gray-900 border border-gray-800 rounded-2xl p-6 w-full max-w-md">
        <div className="flex items-center justify-between mb-5">
          <h2 className="text-white font-bold text-lg">Log Workout</h2>
    ...
    <div className="max-w-5xl mx-auto space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 text-sm mt-0.5">{workouts.length} sessions logged</p>
        </div>
        <button
      ...
      <div className="bg-gray-900 border border-gray-800 rounded-2xl">
        <div className="p-4 border-b border-gray-800">
          <input
            ...
            className="w-full sm:w-64 bg-gray-800 border border-gray-700 rounded-lg px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-orange-500 text-sm"
          />
        </div>
        ...
        {filtered.length === 0 ? (
          <div className="text-center py-16 text-gray-500">
            ...
            {!filter && <button onClick={() => setShowModal(true)} className="text-orange-400 text-sm mt-2 hover:underline">Log your first workout</button>}
          </div>
        ) : (
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-800">
                {filtered.map(w => (
                  <tr key={w.id} className="hover:bg-gray-800/50 transition">
                    <td className="px-4 py-3 text-white font-medium">{w.exercise}</td>
                    <td className="px-4 py-3 text-gray-300">{w.sets}</td>
                    <td className="px-4 py-3 text-gray-300">{w.reps}</td>
                    <td className="px-4 py-3 text-gray-300">{w.weight} kg</td>
                    <td className="px-4 py-3 text-orange-400 font-medium">{(w.sets * w.reps * w.weight).toLocaleString()} kg</td>
